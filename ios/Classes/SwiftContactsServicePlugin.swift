...
public class SwiftContactsServicePlugin: NSObject, FlutterPlugin, CNContactViewControllerDelegate, CNContactPickerDelegate {
        ...
        do {
            try store.enumerateContacts(with: fetchRequest, usingBlock: { (contact, stop) -> Void in
                ...
                if phoneQuery {
                    ...
                    if #available(iOS 11, *) {
                        contacts.append(contact)
                    } else if query != nil && (contact.emailAddresses.contains { labeledValue in
                        let emailString = labeledValue.value as String
                        return emailString.lowercased() == query!.lowercased()
                    }) {
                        contacts.append(contact)
                    }
                } else {
    ...
    @objc func cancelContactForm() {
        if let result = self.result {
            ...
            result(1) // SwiftContactsServicePlugin.FORM_OPERATION_CANCELED
            self.result = nil
        }
    }
         ...
         do {
            // Check to make sure dictionary has an identifier
             guard let identifier = contact["identifier"] as? String else{
                 result(2) // SwiftContactsServicePlugin.FORM_COULD_NOT_BE_OPEN
                 return nil;
             }
            ...
            let cnContact = try store.unifiedContact(withIdentifier: identifier, keysToFetch: keysToFetch).mutableCopy() as! CNMutableContact
    ...
    func dictionaryToContact(dictionary : [String:Any]) -> CNMutableContact{
        ...
        //Simple fields
        // No contact.identifier = ... as it's a get-only property
        contact.givenName = dictionary["givenName"] as? String ?? ""
        contact.familyName = dictionary["familyName"] as? String ?? ""
        contact.middleName = dictionary["middleName"] as? String ?? ""
        contact.namePrefix = dictionary["prefix"] as? String ?? ""
        ...
        if let birthday = dictionary["birthday"] as? String {
            ...
            if let date = formatter.date(from: birthday) { // Added nil check for date
